...
export class PolygonCommand implements ICommand {
    ...
    if (points.length > 0) {
      ...
      if (this.polygon) {
        this.renderer.setAttribute(
          this.polygon,
          "x",
          this.polygonAttributes.x.toString() + "px"
        );
        this.renderer.setAttribute(
          this.polygon,
          "y",
          this.polygonAttributes.y.toString() + "px"
        );
        this.renderer.setAttribute(
          this.polygon,
          "width",
          this.polygonAttributes.width.toString() + "px"
        );
        this.renderer.setAttribute(
          this.polygon,
          "height",
          this.polygonAttributes.height.toString() + "px"
        );
  ...
  execute(): void {
    if (!this.polygon) {
      this.polygon = this.renderer.createElement(
        "polygon",
        "svg"
      ...
      this.renderer.setStyle(
        this.polygon,
        "stroke-width",
        this.polygonAttributes.strokeWidth.toString() + "px"
      );
      this.renderer.setStyle(this.polygon, "fill", this.polygonAttributes.fill);
      this.renderer.setStyle(
        this.polygon,
        "stroke",
        this.polygonAttributes.stroke
      );
      this.renderer.setStyle(
        this.polygon,
        "fillOpacity",
        this.polygonAttributes.fillOpacity.toString()
      );
      this.renderer.setStyle(
        this.polygon,
        "strokeOpacity",
        this.polygonAttributes.strokeOpacity.toString()
      );
