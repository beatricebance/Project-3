...
import { OBJECT_ATTRIBUTE_STRUCTURE } from "../../../models/object-structure.model";
export class ColorApplierCommand implements ICommand {
  ...
  constructor(
    ...
    const propertyMap: Record<string, string> | undefined =
      OBJECT_ATTRIBUTE_STRUCTURE[svgElementName];
    if (!propertyMap) {
      return;
    }
    const actualValue = this.svgElement.style.getPropertyValue(
      propertyMap.primaryColor as string
    );

    /// Texture situation
    if (actualValue.startsWith("url")) {
      const textureElement = document.getElementById(
        actualValue.replace('url("#', "").replace('")', "")
      ) as SVGElement | null;
      if (!textureElement) {
        return;
      }
    ...
    const svgPropertyRecord: Record<string, string> =
      OBJECT_ATTRIBUTE_STRUCTURE[
        this.svgElement.getAttribute("name")
          ? (this.svgElement.getAttribute("name") as string)
          : "rectangle"
      ] as Record<string, string>;
    this.colorAtribute = svgPropertyRecord[colorString] as string;
    this.alphaAtribute = svgPropertyRecord[alphaString] as string;
    this.previousColor = this.svgElement.style.getPropertyValue(
      this.colorAtribute
    );
    this.previousAlpha = this.svgElement.style.getPropertyValue(
      this.alphaAtribute
    );
  }
    ...
    if (markerID) {
      ...
      if (markerEl) {
        this.renderer.setStyle(
          markerEl.firstChild as SVGElement,
          "fill",
          color
        );
        this.renderer.setStyle(
