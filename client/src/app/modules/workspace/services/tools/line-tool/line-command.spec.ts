...
describe("LineCommand", () => {
  ...
  it("#execute should create line svg with marker", () => {
    ...
    lineCommand.execute();
    const l = lineCommand.getLine() as SVGPolylineElement;
    const m = (lineCommand.getMarkerDefs() as SVGDefsElement)
      .firstChild as SVGMarkerElement;
    ...
    expect(m.getAttribute("markerHeight")).toEqual(
      line.diameter.toString() + "px"
    );
    expect(m.getAttribute("markerWidth")).toEqual(
      line.diameter.toString() + "px"
    );
    expect(m.getAttribute("viewBox")).toEqual(
      `0 0 ${line.diameter} ${line.diameter}`
    );
    expect(m.getAttribute("refX")).toEqual(
      (line.diameter / 2).toString() + "px"
    );
    expect(m.getAttribute("refY")).toEqual(
      (line.diameter / 2).toString() + "px"
    );
    expect(m.getAttribute("id")).toEqual(
      `${line.diameter}-Marker-${line.markerId}`
    );
    ...
    expect(l.getAttribute("marker-start")).toEqual(
      `url(#${line.diameter}-Marker-${line.markerId})`
    );
    expect(l.getAttribute("marker-mid")).toEqual(
      `url(#${line.diameter}-Marker-${line.markerId})`
    );
    expect(l.getAttribute("marker-end")).toEqual(
      `url(#${line.diameter}-Marker-${line.markerId})`
    );
  ...
  it("#addPoint after the execute command should update transform", () => {
    ...
    const l = lineCommand.getLine() as SVGPolylineElement;
    expect(l.getAttribute("points")).toEqual(
      `${line.pointsList[0].x} ${line.pointsList[0].y},30 40`
    );
  });
