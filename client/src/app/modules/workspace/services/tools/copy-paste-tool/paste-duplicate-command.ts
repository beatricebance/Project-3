...
export class PasteDuplicateCommand implements ICommand {
  ...
  execute(): void {
    if (this.isPaste) {
      ...
      if (
        this.copyPasteOffsetService.offsetInit.x +
          this.copyPasteOffsetService.OFFSET_CONST >
          this.drawingService.width ||
        this.copyPasteOffsetService.offsetInit.y +
          this.copyPasteOffsetService.OFFSET_CONST >
          this.drawingService.height
      ) {
  ...
  private setNewUrl(object: SVGElement, newId: string): void {
    ...
    if (positionStart !== -1) {
      ...
      const urlId: string = object.outerHTML.substring(
        positionStart + 5,
        positionEnd
      );

      const elementClone: SVGElement = (
        document.getElementById(urlId) as HTMLElement
      ).cloneNode(true) as SVGElement;
      elementClone.id =
        elementClone.id.substring(0, elementClone.id.length - 1) + newId;

      const defElement: SVGDefsElement = this.renderer.createElement(
        "defs",
        "svg"
      );
      ...
      while (positionStart !== -1) {
        ...
        const partToAdd: string = outerHtmlString.substring(
          elementPosition,
          positionStart - 2
        );
