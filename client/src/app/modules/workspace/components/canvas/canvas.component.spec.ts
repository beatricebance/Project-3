...
describe("CanvasComponent", () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        { provide: Renderer2, useValue: rendererSpyTemp },
  ...
  it("should return drawingServicecolor value", () => {
    spyOnProperty(drawingServiceSpy, "rgbaColorString").and.returnValue(
      "rgba(255,0,0,1)"
    );
  ...
  it(" ngAfterView should be called with no svg", () => {
    ...
    expect(rendererSpy.appendChild).toHaveBeenCalledWith(
      component.canvasDiv.nativeElement,
      el
    );
  });

  it(" ngAfterView should be called with svg", () => {
    const previousel: SVGElement = document.createElementNS(
      "svg",
      "svg"
    ) as SVGElement;
    const el: SVGElement = document.createElementNS("svg", "svg") as SVGElement;
